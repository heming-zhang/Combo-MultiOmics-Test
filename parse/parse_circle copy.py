...
class GeneAnnotation():
    ...
    def kegg_omics_intersect(self):
        ...
        rna_df = rna_df.sort_values(by=['symbol'])
        rna_gene_list = list(rna_df['symbol'])
        cmeth_df = pd.read_csv('../datainfo/init_data/ccle_methylation.csv', low_memory=False)
        cmeth_df = cmeth_df.sort_values(by=['locus_id'])
        cmeth_gene_list = list(cmeth_df['locus_id'])
        omics_gene_list = sorted(list(set(rna_gene_list).intersection(set(cmeth_gene_list))))
        ...
        kegg_omics_gene_df = kegg_omics_gene_df.dropna().reset_index(drop=True) # [8002] => [4260] GENEs
        # TAIL [KEGG] GENE
        ...
        # TAIL [GDSC RNASeq]
        new_rna_df = rna_df[rna_df['symbol'].isin(new_kegg_gene_list)].reset_index(drop=True)
        new_rna_df.to_csv('../datainfo/mid_gene/tail_gene_rna.csv', index=False, header=True)
        # TAIL [CCLE Methylation]
        new_cmeth_df = cmeth_df[cmeth_df['locus_id'].isin(new_kegg_gene_list)].reset_index(drop=True)
        new_cmeth_df.to_csv('../datainfo/mid_gene/tail_gene_cmeth.csv', index=False, header=True)
...
class CellLineAnnotation():
    ...
    def omics_cell(self):
        ...
        # # # [CCLE Methylation]
        tail_gene_cmeth_df = pd.read_csv('../datainfo/mid_gene/tail_gene_cmeth.csv', low_memory=False)
        cmeth_cell_line_list = sorted(list(tail_gene_cmeth_df.columns)[1:])
        ...
        # # # [GDSC CNV]
        tail_gene_rna_df = pd.read_csv('../datainfo/mid_gene/tail_gene_rna.csv', low_memory=False)
        rna_cell_line_list = sorted(list(tail_gene_rna_df.columns)[1:])
        ...
        dl_cmeth_rna_cell_annotation_df.to_csv('../datainfo/init_data/omics_cell_annotation.csv', index=False, header=True)
    ...
    def tail_cell(self):
        ...
        tail_gene_rna_df = pd.read_csv('../datainfo/mid_gene/tail_gene_rna.csv', low_memory=False)
        tail_gene_cell_rna_df = tail_gene_rna_df[omics_rna_cell_line_list]
        nci_rna_cell_line_dict = dict(zip(omics_cell_df.rna_cell, omics_cell_df.dl_cell))
        tail_gene_cell_rna_df = tail_gene_cell_rna_df.rename(columns=nci_rna_cell_line_dict)
        tail_gene_cell_rna_df.insert(0, 'symbol', list(tail_gene_rna_df['symbol']))
        tail_gene_cell_rna_df.to_csv('../datainfo/mid_cell_line/tail_gene_cell_rna.csv', index=False, header=True)
        ...
        tail_gene_cmeth_df = pd.read_csv('../datainfo/mid_gene/tail_gene_cmeth.csv', low_memory=False)
        tail_gene_cell_cmeth_df = tail_gene_cmeth_df[omics_cmeth_cell_line_list]
        nci_cmeth_cell_line_dict = dict(zip(omics_cell_df.cmeth_cell, omics_cell_df.dl_cell))
        tail_gene_cell_cmeth_df = tail_gene_cell_cmeth_df.rename(columns=nci_cmeth_cell_line_dict)
        tail_gene_cell_cmeth_df.insert(0, 'locus_id', list(tail_gene_cmeth_df['locus_id']))
        tail_gene_cell_cmeth_df.to_csv('../datainfo/mid_cell_line/tail_gene_cell_cmeth.csv', index=False, header=True)
