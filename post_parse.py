...
def input_random():
    final_input_df = pd.read_csv('./datainfo/filtered_data/final_dl_input.csv')
    random_final_input_df = final_input_df.sample(frac = 1)
    random_final_input_df.to_csv('./datainfo/filtered_data/random_final_dl_input.csv', index = False, header = True)

# SPLIT DEEP LEARNING INPUT INTO TRAINING AND TEST
def split_k_fold(k):
    random_final_dl_input_df = pd.read_csv('./datainfo/filtered_data/random_final_dl_input.csv')
    ...
    for place_num in range(k):
        ...
        split_input_df.to_csv('./datainfo/filtered_data/split_input_' + str(place_num + 1) + '.csv', index = False, header = True)


# input_random()
# split_k_fold(k=5)

if os.path.exists('./datainfo/form_data') == False:
    os.mkdir('./datainfo/form_data')
k = 5
batch_size = 64
dir_opt = '/datainfo'
LoadData(dir_opt).load_all_split(batch_size, k)

################ MOUDLE 2 ################
dir_opt = '/datainfo'
LoadData(dir_opt).load_adj_edgeindex()
...
n_fold = 1
dir_opt = '/datainfo'
LoadData(dir_opt).load_train_test(k, n_fold)